...
const unsigned char utgb_battle_attack_bg_tilemap[] = {
    0x20, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x24, 0x27, 0x28, 0x29, 0x20, 0x20,
    0x2A, 0x2B, 0x2C, 0x2C, 0x2D, 0x2E, 0x2F, 0x30, 0x31, 0x32, 0x33, 0x33, 0x34, 0x35,
    0x36, 0x37, 0x38, 0x38, 0x39, 0x3A, 0x3B, 0x3C, 0x3D, 0x3E, 0x3F, 0x3F, 0x40, 0x41,
    0x20, 0x20, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x45, 0x48, 0x49, 0x4A, 0x20, 0x20
};

const unsigned char utgb_battle_attack_bg_tilemap_attr[] = {
    0x02, 0x02, 0x02, 0x02, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x02, 0x02, 0x02, 0x02,
    0x02, 0x02, 0x02, 0x02, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x02, 0x02, 0x02, 0x02,
    0x02, 0x02, 0x02, 0x02, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x02, 0x02, 0x02, 0x02,
    0x02, 0x02, 0x02, 0x02, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x02, 0x02, 0x02, 0x02
};
...
void utgb_draw_battle_attackbg(void) BANKED {
    ...
    if (_is_CGB) {
        VBK_REG = 1;
        set_bkg_tiles(3, 10, 14, 4, utgb_battle_attack_bg_tilemap_attr);
        VBK_REG = 0;
    }
#endif
...
void utgb_clear_battle_attackbg(void) BANKED {

    fill_bkg_rect(3, 10, 14, 4, (text_palette & 0x07u));

#ifdef CGB
    if (_is_CGB) {
        VBK_REG = 1;
        fill_bkg_rect(2, 9, 16, 6, (text_palette & 0x07u));
        VBK_REG = 0;
    }
#endif
...
void utgb_draw_frame_bkg(UBYTE x, UBYTE y, UBYTE width, UBYTE height) BANKED {
    ...
    if (_is_CGB) {
        //VBK_REG = 1;
        //fill_bkg_rect(x, y, width, height, (text_palette & 0x07u));
        //VBK_REG = 0;
    }
#endif
